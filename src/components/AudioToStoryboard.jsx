    ...
    try {
      ...
      const resScene = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${apiKey.trim()}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              { text: `Kamu adalah Sutradara Storyboard. Cerita: "${storySummary}". Karakter yang ada: ${characters.join(', ')}. Lokasi: ${locations.join(', ')}.\nTugas: Buat storyboard berkesinambungan bergaya sinematik layar lebar (bukan kartun). Total Durasi: ${duration} detik. Buat tepat ${totalScenes} Scene (masing-masing 10 detik).\nDi SETIAP Scene, WAJIB buat tepat 6 panel/shot (TIDAK BOLEH lebih atau kurang dari 6 panel per grid).\n\nPENTING: Di SETIAP Panel, kamu WAJIB menambahkan prompt visual teknis berbahasa Inggris di akhir deskripsi agar gambar yang dihasilkan benar-benar sinematik, pakaian konsisten 100% dengan referensi, dan bukan kartun/3d. Contoh format penulisan Panel:\nPanel 1: [Deskripsi adegan bahasa Indonesia]. Cinematic shot, 35mm lens, photorealistic, highly detailed, live-action movie style, NOT cartoon, NO 3d render, characters wearing exact consistent clothing as reference.\n\nLakukan ini untuk SEMUA panel. Jangan sampai ada gambar kartun.\n\nFormat Output:\nScene 1 (0s-10s) - Lokasi: ...\nPanel 1: ...\nPanel 2: ...\n(sampai Panel 6)\n\nScene 2 (10s-20s) - Lokasi: ...\n(sampai Scene ${totalScenes})` }
            ]
          }],
